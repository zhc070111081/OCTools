...
@interface HMWebView ()<WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate>

/// webView
@property (nullable, strong, nonatomic) WKWebView *wkWebView;
...
- (void)setUserContentController {
    ...
    self.wkWebView.configuration.userContentController = content;
}
...
//    [self.wkWebView addObserver:self forKeyPath:NSStringFromSelector(@selector(estimatedProgress)) options:0 context:nil];
    [self.wkWebView addObserver:self forKeyPath:KEstimatedProgress options:0 context:nil];
    [self.wkWebView addObserver:self forKeyPath:KTitle options:0 context:nil];
}
 ...
 */
- (void)removeObserver {
    [self.wkWebView removeObserver:self forKeyPath:KEstimatedProgress context:nil];
    [self.wkWebView removeObserver:self forKeyPath:KTitle context:nil];
}
