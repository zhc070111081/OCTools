//
//  HMUncaughtExceptionHandler.m
//  OCToolsClass
//
//  Created by zhuhc on 2021/8/26.
//

#import "HMUncaughtExceptionHandler.h"
...
/// max save log days
static NSInteger const HMCrashMAXSaveDay = 7;

/// log file folder name
static NSString *const HMCrashFolderName = @"CrashLOG";


static HMUncaughtExceptionHandler *_handler = nil;

@interface HMUncaughtExceptionHandler ()
...
@end
@implementation HMUncaughtExceptionHandler
...
+ (instancetype)defaultExceptionHandler {
    ...
    dispatch_once(&once, ^{
        _handler = [[HMUncaughtExceptionHandler alloc] init];
    });
...
- (void)checkLogSaveDay {
    ...
    for (NSString *fileName in files) {
        ...
        if (components.day > HMCrashMAXSaveDay) {
            // 删除大于7天的日志
...
- (NSString *)absoluteFolderPath {
    return [NSHomeDirectory() stringByAppendingString:[NSString stringWithFormat:@"/Documents/%@",HMCrashFolderName]];
}
...
//- (HMUncaughtExceptionHandler *(^)(BOOL isShow))showAlert{
//    return ^(BOOL isShow) {
//        self.isShowAlert = isShow;
//        return [HMUncaughtExceptionHandler defaultExceptionHandler];
//    };
//}
//
//- (HMUncaughtExceptionHandler *(^)(BOOL isShow))showErrorInfor{
//    return ^(BOOL isShow) {
...
//}
//
//- (HMUncaughtExceptionHandler *(^)(void(^ logPathBlock)(NSString *pathStr)))getlogPathBlock{
...
void HandleException(NSException *exception){
    ...
    [[HMUncaughtExceptionHandler defaultExceptionHandler] performSelectorOnMainThread:@selector(handleException:) withObject:[NSException exceptionWithName:[exception name] reason:[exception reason] userInfo:userInfo] waitUntilDone:YES];
}
...
void SignalHandler (int signal){
    ...
    NSArray *callStack = [HMUncaughtExceptionHandler backtrace];
    [userInfo setObject:callStack forKey:UncaughtExceptionHandlerAddressesKey];
